...
<script lang="ts">
	...
	export let yArray: Y.Array<KanbanItem>;
	export let onAddCard: ((columnId: string) => void) | undefined = undefined;
	export let onCardDrop: ((cardId: string, targetColumnId: string) => void) | undefined = undefined;
...
>
	...
	{#if onAddCard}
		<button class="add-card-btn" on:click={() => onAddCard?.(columnId)}>+ Add Card</button>
	{/if}
</div>
